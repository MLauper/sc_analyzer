...
bool compare_tracks(dto::Track i, dto::Track j) { return (i.overall_suggestion.at(1).likelihood > j.overall_suggestion.at(1).likelihood); }
                                                          ...
                                                          std::vector<dto::Person>& persons)
{
	...
	for (auto& t : tracks)
	{
		if (tracks.size() > 1)
		{
			...
			{
				if (dto::Configuration::PRINT_PERSON_SELECTOR_STEPS) std::cout << "Create new Person " << (i) << " for Tracks " << t.camera.scene << "." << t.trackId << " and " << match->camera.scene << "." << match->trackId << std::endl;
				int assignment = i++;
			...
			{
				if (dto::Configuration::PRINT_PERSON_SELECTOR_STEPS) std::cout << "Assign ID " << match->assignedPersonId << " to Track " << t.camera.scene << "." << t.trackId << " from Track " << match->camera.scene << "." << match->trackId << std::endl;
				t.assignedPersonId = match->assignedPersonId;
			}
			// Use person assignment of track if only track has one
			else if (t.assignedPersonId != -1 && match->assignedPersonId == -1)
			{
				if (dto::Configuration::PRINT_PERSON_SELECTOR_STEPS) std::cout << "Assign ID " << t.assignedPersonId << " to Track " << match->camera.scene << "." << match->trackId << " from Track " << t.camera.scene << "." << t.trackId << std::endl;
			...
			else
			{
				if (t.overall_suggestion.at(1).likelihood > dto::Configuration::ALWAYS_MATCH_LIKELIHOOD)
				{
					...
					if (replace_id != replacement_id) {
						if (dto::Configuration::PRINT_PERSON_SELECTOR_STEPS) std::cout << "Merge Tracks " << t.camera.scene << "." << t.trackId << " and " << match->camera.scene << "." << match->trackId << ", IDs: " << t.assignedPersonId << " and " << match->assignedPersonId << std::endl;
						for (auto& x : tracks) if (x.assignedPersonId == replace_id) x.assignedPersonId = replacement_id;
					} else
					{
						if (dto::Configuration::PRINT_PERSON_SELECTOR_STEPS) std::cout << "Tracks " << t.camera.scene << "." << t.trackId << " and " << match->camera.scene << "." << match->trackId << ", IDs: " << t.assignedPersonId << " and " << match->assignedPersonId << " already in same group, do nothing." << std::endl;
					}
				}
				else {
					if (dto::Configuration::PRINT_PERSON_SELECTOR_STEPS) std::cout << "Do not touch id on tracks " << t.camera.scene << "." << t.trackId << " and " << match->camera.scene << "." << match->trackId << std::endl;
				}
			}
		} 
