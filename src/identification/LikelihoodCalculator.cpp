	...
	for (auto& t : tracks)
	{
		for (int i = 0; i < tracks.size(); i++)
		{
			...
			overall_likelihood += t.surf_keypoint_suggestion.at(i).likelihood * dto::Configuration::
				surf_keypoint_suggestion_weight;
			overall_likelihood += t.sift_keypoint_suggestion.at(i).likelihood * dto::Configuration::
				sift_keypoint_suggestion_weight;
			overall_likelihood += t.upper_body_color_suggestion.at(i).likelihood * dto::Configuration::
				upper_body_color_suggestion_weight;
			overall_likelihood += t.lower_body_color_suggestion.at(i).likelihood * dto::Configuration::
				lower_body_color_suggestion_weight;
		...
		sort(t.size_width_suggestion.begin(), t.size_width_suggestion.end(), compare_suggestions);
		sort(t.size_height_suggestion.begin(), t.size_height_suggestion.end(), compare_suggestions);
		sort(t.surf_keypoint_suggestion.begin(), t.surf_keypoint_suggestion.end(), compare_suggestions);
		sort(t.sift_keypoint_suggestion.begin(), t.sift_keypoint_suggestion.end(), compare_suggestions);
		sort(t.upper_body_color_suggestion.begin(), t.upper_body_color_suggestion.end(), compare_suggestions);
		sort(t.lower_body_color_suggestion.begin(), t.lower_body_color_suggestion.end(), compare_suggestions);
		sort(t.overall_suggestion.begin(), t.overall_suggestion.end(), compare_suggestions);

		if (dto::Configuration::SAVE_TRACK_STATISTICS)
		{
			std::ofstream fileStream;
			std::stringstream filePath;
			filePath << dto::Configuration::STATISTICS_DIRECTORY << "scene-" << t.camera.scene << "\\" << t.camera.prefix << "\\"
				<< "Track-" << t.trackId << "_statistics.txt";
			...
			{
				fileStream << "  " << s.likelihood << ":  Scene " << s.track->camera.scene << ", Track " << s.track->trackId <<
					", Prefix " << s.track->camera.prefix << std::endl;
			}
