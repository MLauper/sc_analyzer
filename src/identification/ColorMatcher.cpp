...
void identification::ColorMatcher::matchAllColors(std::vector<dto::Track>& tracks) const
{
	for (auto& t : tracks)
	{
		for (auto& comp : tracks)
		{
			...
			const auto diffColorUpperBody = diffColorUpperBody_1 < diffColorUpperBody_2
				                                 ? diffColorUpperBody_1
				                                 : diffColorUpperBody_2;

			auto probUpperBodyColor = 1 - diffColorUpperBody / 2.0f;
			...
			const auto diffColorLowerBody = diffColorLowerBody_1 < diffColorLowerBody_2
				                                 ? diffColorLowerBody_1
				                                 : diffColorLowerBody_2;

			auto probLowerBodyColor = 1 - diffColorLowerBody / 2;
